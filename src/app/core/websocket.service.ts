...
export class WebSocketService {
     ...
     */
    public onConnectStateChange: (result: boolean, code?: number, reason?: string) => void = null;
    /**
     ...
     * @private
     */
    private portNumberGenerator: (skip: number, count: number) => number[] = null;
     ...
     * @private
     */
    private commandCallbacks: Map< string, { success: (data: any) => void, failure: (reason?: any) => void }> = new Map<string, { success: (data: any) => void, failure: (reason?: any) => void }>();
     ...
     */
    public startWebSocketConnection(portGenerator: (skip: number, count: number) => number[], maxPorts: number, singleLoop?: boolean): void {
            ...
            try {
                if (this.webSocket) {
                    if (this.webSocket.readyState === this.webSocket.OPEN) {
                        msg.internal_command_uuid = UUID.UUID();
                        this.commandCallbacks.set(msg.internal_command_uuid, { success : resolve, failure : reject});
    ...
    public sendBinary<T>(command: any, frameType: BinaryFrameCommand, data: Uint8Array): Promise<T> {
        return new Promise<T>( (resolve: (value?: T) => void, reject: (reason?: any) => void) => {
            try {
                if (this.webSocket) {
                    if (this.webSocket.readyState === this.webSocket.OPEN) {
                        ...
                        const header = new Uint8Array(this.str2ab(JSON.stringify(command)));
                        ...
                        //  data size : 4 bytes
                        //  data : dn bytes
                        const frame: Uint8Array = new Uint8Array( 1 + 4 + header.byteLength + 4 + data.byteLength);
                        ...
                        // setup data size
                        frame.set(this.numberToUint8Array(data.byteLength), 5 +  header.length);

                        // setup data
                        frame.set(data, 5 +  header.length + 4);

                        this.commandCallbacks.set(uuid, { success : resolve, failure : reject});
     ...
     * @private
     */
    private connectionAttempt_(port: number, successCallback: (ws: WebSocket, port: number) => void , errorCallback: () => void) {
     ...
     * @private
     */
    private connectionAttempts_(ports: number[], successCallback: (ws: WebSocket) => void , errorCallback: () => void) {
        let attemptFailCount = 0;
        ports.forEach((port: number) => {
            this.connectionAttempt_(port,
                (websocket: WebSocket) => {
                    attemptFailCount = 0;
     ...
     * @private
     */
    private attemptNext_(portIndex: number, count: number, callback: (ws: WebSocket) => void) {
        ...
        ws.onmessage = (evt: MessageEvent) => {
            if (evt.data instanceof Blob) {
                ...
                fileReader.onloadend = (ev: ProgressEvent) => {
                    ...
                    // send result
                    this.notifySuccess(commandUuid, new Blob([content.subarray(2 + content[1])], {type: 'application/octet-stream'}));
                };
    ...
    private notifySuccess<T>(uuid: string, data: T) {
        if (uuid != null && this.commandCallbacks.has(uuid) && this.commandCallbacks.get(uuid) != null) {
